...
overview_data = [
    ...
    ("王政鈞", "视觉模型优化（遮挡场景）· 数据扩充", "v15 模型权重、遮挡对比指标报告"),
    ("刘炳尧", "机械臂结构学习 · 仿真协作", "仿真联调测试数据、结构调研报告"),
...
method(
    doc,
    '与视觉模块约定接口格式：接收 dict {"x": float, "y": float, "z": float}，单位 mm',
)
...
# 王政鈞
heading3(doc, "王政鈞 — 标注质量检查 · 为第二阶段优化准备")
...
task_label(doc, "② 当前阶段不更换模型权重，配合联调使用 v14 完成推理验证")
method(
...
# 刘炳尧
heading3(doc, "刘炳尧 — 机械臂结构学习 · 仿真协作")

task_label(doc, "① 学习 SO-101 机械臂结构与运动学基础")
method(doc, "阅读 LeRobot 仓库文档，了解 SO-101 的关节定义、坐标系约定和自由度")
method(
    doc,
    "安装 MuJoCo 并加载 SO-101 MJCF 模型：pip install mujoco，"
    '运行 python -c "import mujoco; print(mujoco.__version__)" 验证',
)
method(doc, "在 MuJoCo 查看器中手动拖动关节，理解各关节运动范围和末端位置变化")

task_label(doc, "② 协助陆坚其搭建仿真场景，参与接口对接调试")
method(doc, "协助在 MJCF 场景中添加目标物体（用圆柱体模拟草莓），放置在固定坐标")
method(doc, '与陆坚其核对接口约定格式：{"x": float, "y": float, "z": float}，单位 mm')
method(doc, "记录仿真调试过程中遇到的问题，整理成学习笔记备用")
...
task_label(doc, "② 梳理论文创新点，与王政鈞对齐实验设计")
method(
    doc,
...
# 王政鈞
heading3(doc, "王政鈞 — 遮挡标注修正 · 数据增强 · v15 训练")
...
method(doc, "运行训练脚本：python vision/train.py")
...
# 刘炳尧
heading3(doc, "刘炳尧 — 仿真场景完善 · 采摘顺序协作测试")

task_label(doc, "① 协助完善 MuJoCo 仿真场景")
method(
    doc, "在 MJCF 中添加草莓植株模型（可用多个圆柱体+球体组合近似）、桌面、多颗草莓目标"
)
method(doc, "设置合理的摩擦系数和碰撞参数，确保抓取时草莓不会异常弹飞")

task_label(doc, "② 参与采摘顺序算法测试，负责测试用例准备与数据记录")
method(doc, "准备不同空间分布的草莓目标配置（密集/稀疏/遮挡），供陆坚其的算法压测")
method(doc, "记录每种配置下算法的采摘成功率、碰撞次数、用时，整理成对比表格")

task_label(doc, "③ 调研机械臂运动规划相关文献，输出文献摘要")
method(doc, "重点关注：SO-ARM100 的实际使用案例、LeRobot 社区的部署经验")
method(doc, "输出 2–3 篇文献摘要，供第三阶段论文写作参考")
...
iface_data = [
    (
        "王政鈞\n（视觉模型）",
        "视觉推理\n（全员可用）",
        "更新 best.pt 路径，在 vision/inference.py 顶部 WEIGHTS 变量修改",
        "不涉及数据格式",
    ),
    (
        "视觉推理\n（inference.py）",
...
# 保存
# ════════════════════════════════════════════════════════════
out_path = r"C:\Users\xzKan\Desktop\机械臂草莓\docs\项目任务分工_v3.docx"
doc.save(out_path)
